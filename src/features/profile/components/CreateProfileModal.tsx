...
interface Props {
  ...
  lang: "hi" | "en";
  onComplete?: (profile: GeneratedProfile) => void;
}
...
type Step = "voice" | "preview" | "success";
type VoiceState = "idle" | "listening" | "processing" | "done";

const steps: Step[] = ["voice", "preview", "success"];

export function CreateProfileModal({ open, onOpenChange, lang, onComplete }: Props) {
  const txt = t[lang];
          ...
          {stepLabels.map((label, i) => (
            <div key={label} className="flex items-center gap-1 sm:gap-2">
              <div className="flex flex-col items-center gap-0.5 sm:gap-1">
                <div
                  className={`flex h-6 w-6 sm:h-8 sm:w-8 items-center justify-center rounded-full text-[10px] sm:text-xs font-bold transition-colors ${
                    i <= stepIndex
              ...
              {i < stepLabels.length - 1 && (
                <div
                  className={`h-0.5 w-3 sm:w-8 shrink-0 transition-colors ${
              ...
              <div className="flex flex-col items-center py-1 sm:py-4">
                ...
                >
                  ...
                  {voiceState === "processing" ? (
                    ...
                    <MicOff className="h-7 w-7 sm:h-10 sm:w-10" />
                  ) : (
